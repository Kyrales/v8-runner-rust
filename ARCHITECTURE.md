...
- Current implementation gap: CLI EDT execution still creates direct non-shared interactive `1cedtcli` sessions when `tools.edt_cli.interactive_mode=true` for `init`, EDT export during `build`, and CLI `syntax edt`; these sessions already inherit the common timeout/cancellation policy, but target architecture remains shared interactive EDT through a common actor/manager per ADR-0007.
