...
Команда сканирует текущий каталог, опирается на файлы-маркеры и их содержимое: ищет Designer-исходники по `Configuration.xml`, EDT-проекты по `.project`, определяет ordinary EDT `source-set[].type` по `V8ConfigurationNature` / `V8ExtensionNature`, читает runtime version из `DT-INF/PROJECT.PMF`, а native EDT layout подтверждает по `src/Configuration/Configuration.mdo`. Для `EXTENSION` generated `source-set[].name` берётся из имени конфигурации расширения в `Configuration.xml` / `src/Configuration/Configuration.mdo`, потому что это имя передаётся платформе как `-Extension`. Для EDT `CONFIGURATION` runtime version записывается в `tools.platform.version`. Отсутствие `Base-Project` у `EXTENSION` не блокирует autodiscovery, но выводится как warning. Для Designer external root нужен однородный набор top-level XML дескрипторов, для EDT external root — однородные direct child projects одного external-kind с валидными `.project`, `DT-INF/PROJECT.PMF`, `Base-Project` и canonical `src/root.xml`. Mixed/ambiguous external roots не автогенерируются. Если autodiscovery не нашёл ни одного `CONFIGURATION` или external source-set требует `--builder DESIGNER`, команда завершится validation error вместо записи phantom-конфига.
