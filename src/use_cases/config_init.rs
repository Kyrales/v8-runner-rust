...
fn render_config(
    ...
    yaml.push_str("# Maximum execution time for a v8-runner command, in milliseconds.\n");
    yaml.push_str("execution_timeout: 3600000\n");
...
mod tests {
    use super::{
        discover_sources, execute, render_config, ConfigBuilderRequest, ConfigFormatRequest,
        ConfigInitRequest, SourcePurpose,
    };
    ...
    fn creates_config_from_designer_sources() {
        ...
        let config_text =
            std::fs::read_to_string(dir.path().join("v8project.yaml")).expect("config");
        assert!(config_text
            .contains("# Maximum execution time for a v8-runner command, in milliseconds."));
        assert!(config_text.contains("execution_timeout: 3600000"));
        assert!(config_text.contains("name: 'SalesAddon'"));
        assert!(config_text.contains("type: EXTENSION"));
    }

    #[test]
    fn render_config_writes_one_hour_global_execution_timeout_with_comment() {
        let yaml = render_config(
            None,
            ConfigFormatRequest::Designer,
            ConfigBuilderRequest::Designer,
            &[],
            None,
        );

        assert!(yaml.contains("# Maximum execution time for a v8-runner command, in milliseconds."));
        assert!(yaml.contains("execution_timeout: 3600000"));
    }
