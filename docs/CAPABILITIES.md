...
- Для ordinary EDT type `CONFIGURATION`/`EXTENSION` выводится из `.project` natures (`V8ConfigurationNature` / `V8ExtensionNature`), runtime version берётся из `DT-INF/PROJECT.PMF`, а valid native layout подтверждается `src/Configuration/Configuration.mdo`. Для `EXTENSION` generated `source-set[].name` берётся из имени конфигурации расширения в `Configuration.xml` / `src/Configuration/Configuration.mdo`. Runtime version EDT `CONFIGURATION` попадает в generated `tools.platform.version`. Если у `EXTENSION` отсутствует `Base-Project`, `config init` всё равно добавляет source-set, но выводит warning.
