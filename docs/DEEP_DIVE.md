...
Подробное руководство по тому, как `v8-test-runner` работает после этапа быстрого старта.
...
2. экспортировать затронутые EDT-проекты во временные файлы в формате Designer под `workPath`;
...
`test all` и `test module <NAME>` — это оркестрационные команды, а не отдельные команды запуска тестов.
...
4. разбирают JUnit XML и логи YaXUnit в структурированный вывод.
...
Механика сохранения артефактов намеренно устроена именно так. Если выполнение упало или разбор отчёта сломался, система сохраняет папку запуска под `workPath/temp/yaxunit/runs/<run-id>/` для отладки, а CLI/MCP-диагностика при этом санитизируется перед показом наружу.
...
- `workPath/temp/yaxunit/`: временные и сохранённые тестовые артефакты
...
Если вам нужна точная публичная поверхность, используйте [CAPABILITIES.md](CAPABILITIES.md). Если нужна внутренняя карта модулей, используйте [../ARCHITECTURE.md](../ARCHITECTURE.md).
